<?php
$script_handle = "flexible-markets_comprehensive_protection-js";
wp_enqueue_script(
    $script_handle,
    get_template_directory_uri() . "/js/partials-min/flexible-markets_comprehensive_protection.min.js",
    array("jquery"),
    null,
    true
);
...
if(get_sub_field('enable_comprehensive_protection')):
    $protection = get_sub_field('comprehensive_protection');
?>
        ...
        <div class="row">
            <div class="col-12 mb-0 mb-md-4">
                <div class="logo-contain">
                    <?= wp_get_attachment_image( $protection['logo']['ID'], 'full', false, array( 
                        'class' => 'd-none d-md-block', 
                        'loading' => 'lazy', 
                        'decoding' => 'async' 
                    ) ); ?>
                    <?= wp_get_attachment_image( $protection['logos_movil']['ID'], 'full', false, array( 
                    ...
                    <a href="<?= get_field('post_url', $item['comprehensive_protection_item']->ID); ?>" target="_blank">
                        <div class="card-post">
                            <div class="image-contain">
                                <?= get_the_post_thumbnail($item['comprehensive_protection_item']->ID, 'large', array( 
                                    'class' => 'feature-image', 
                                    'loading' => 'lazy', 
                                    'decoding' => 'async' 
                                )); ?>
                                <span class="icon">
                                    <?= wp_get_attachment_image( get_field('icon', $item['comprehensive_protection_item']->ID)['ID'], 'full', false, array( 
                                        'class' => 'icon-image', 
                                        'loading' => 'lazy', 
                                        'decoding' => 'async' 
                                    ) ); ?>
