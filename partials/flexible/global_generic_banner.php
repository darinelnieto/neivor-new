<?php
$script_handle = "flexible-global_generic_banner-js";
wp_enqueue_script(
    $script_handle,
    get_template_directory_uri() . "/js/partials-min/flexible-global_generic_banner.min.js",
    array("jquery"),
    null,
    true
);
...
$background = get_sub_field('background_gradient');
$background_desktop = get_sub_field('desktop_background');
$background_movil = get_sub_field('movil_background');
$validate_background = get_sub_field('enable_background_image');
$desktop_image = get_sub_field('desktop_size_main_image');
$movil_image = get_sub_field('movil_size_main_image');
$cta = get_sub_field('cta_link');
$enable_hsform = get_sub_field('enable_form_hs');
$hsform = get_sub_field('script_form');
$show_breadcrumbs = get_sub_field('show_breadcrumbs') === true;
$enable_image_form_hs_ = get_sub_field('enable_hs_form_image');
$hs_form_image2 = get_sub_field('hs_form_image');
$enable_move_image_to_right_place_hs_ = get_sub_field('enable_move_image_to_right_place_hs');

if(get_sub_field('custom_style_image_desktop') === true):
?>
<style>
    .generic-banner-partial-ec8c2e .img-desktop{
        max-width:<?= get_sub_field('max_width_desktop'); ?> !important;
        margin:<?= get_sub_field('margin_desktop'); ?> !important;
    }
	html {
...
<?php 
    ...
    if(get_sub_field('custom_style_image_movil') === true):
?>
<style>
    .generic-banner-partial-ec8c2e .img-movil{
        max-width:<?= get_sub_field('max_width_movil'); ?> !important;
        margin:<?= get_sub_field('margin_movil'); ?> !important;
    }
</style>
<?php endif; ?>
<section class="generic-banner-partial-ec8c2e" <?php if($validate_background === false): ?>style="<?= $background; ?>"<?php endif; ?>>
    <div class="banner">
        ...
        <?php if($validate_background === true): ?>
            <?= wp_get_attachment_image( $background_desktop['ID'], 'full', false, array(
                'class' => 'background-desktop',
                'fetchpriority' => 'high',
            )); ?>
            <?= wp_get_attachment_image( $background_movil['ID'], 'full', false, array(
        ...
        <div class="container">
            <div class="row">
                <div class="col-12 col-md-6 mb-4 mb-md-0 text-content">
                    ...
                    <?php endif; ?>
                    <?php if(!empty(get_sub_field('main_text'))): ?>
                        <h1 class="title" style="color:<?= get_sub_field('text_color'); ?><?php if(get_sub_field('enable_font_size_by_title') === true): ?>; font-size:<?= get_sub_field('font_size_title'); ?><?php endif; ?>">
                            <?= get_sub_field('main_text'); ?>
                        </h1>
                    <?php endif; if(!empty(get_sub_field('complement_text'))): ?>
                    <div class="description" style="margin-bottom: 30px; color:<?= get_sub_field('text_color'); ?>">
                        <?= get_sub_field('complement_text'); ?>
                    </div>
                    <?php endif; ?>
                    <?php if(get_sub_field('add_cta_link') === true && $cta): ?>
                        <a href="<?= $cta['url']; ?>" class="d-none d-md-flex cta" target="<?= $cta['target']; ?>" style="color:<?= get_sub_field('text_color_cta'); ?>; background:<?= get_sub_field('background_cta'); ?>">
                            <span><?= $cta['title']; ?></span>
                        </a>
                    <?php 
                        endif;
                        if(get_sub_field('add_text_after_the_cta')):
                    ?>
                        <div class="text-after-cta d-none d-md-block" style="color:<?= get_sub_field('complement_text_color_movil'); ?>">
                ...
                <div class="col-12 col-md-6 image-contain">
                    <?php if(get_sub_field('enable_video') === false): ?>
                        <div class="image-contain">
                            <?= wp_get_attachment_image( $desktop_image['ID'] ?? '', 'full', false, array(
                                'class' => 'd-none d-md-block img-desktop',
                                'fetchpriority' => 'high',
                            )); ?>
                            <?= wp_get_attachment_image( $movil_image['ID'] ?? '', 'full', false, array(
                    ...
                    <div class="text-content d-block d-md-none">
                        ...
                        <?php 
                            ...
                            if(get_sub_field('add_text_after_the_cta')):
                        ?>
                            <div class="text-after-cta" style="color:<?= get_sub_field('complement_text_color_movil'); ?>">
                                <p><?= get_sub_field('text_after_the_cta'); ?></p>
                            </div>
                        <?php endif; ?>
                    </div>
    ...
    <?php
        ...
        if(get_sub_field('add_nav') && $nav):
            $styles = get_sub_field('enable_styles');
    ?>
