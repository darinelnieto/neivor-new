<?php
$script_handle = "globals-generic-banner-js";
wp_enqueue_script(
    $script_handle,
    get_template_directory_uri() . "/js/partials-min/globals-generic-banner.min.js",
    array("jquery"),
    null,
    true
);
...
$show_breadcrumbs = get_field('show_breadcrumbs') === true;
...
<section class="generic-banner-partial-ec8c2e" <?php if($validate_background === false): ?>style="<?= $background; ?>"<?php endif; ?>>
    <div class="banner">
        ...
        <?php if($validate_background === true): ?>
            <?= wp_get_attachment_image($background_desktop['ID'] ?? '', 'large', false, array(
                'class' => 'background-desktop',
                'fetchpriority' => 'high',
                'loading' => 'eager'
            )); ?>
            <?= wp_get_attachment_image($background_movil['ID'] ?? '', 'large', false, array(
                ...
                <div class="col-12 col-md-6 image-contain">
                    <?php if(get_field('enable_video') === false): ?>
                        <div class="image-contain">
                            <?= wp_get_attachment_image($desktop_image['ID'] ?? '', 'large', false, array(
                    ...
                    <div class="text-content d-block d-md-none">
                        <?php if(get_field('add_cta_link') === true && $cta): ?>
                            <a href="<?= $cta['url']; ?>" class="cta d-flex d-md-none my-5" target="<?= $cta['target']; ?>" style="color:<?= get_field('text_color_cta'); ?>; background:<?= get_field('background_cta'); ?>">
                                <?= $cta['title']; ?>
                            </a>
                        <?php 
                            endif;
