<?php
$script_handle = "about-banner-about-js";
wp_enqueue_script(
    $script_handle,
    get_template_directory_uri() . "/js/partials-min/about-banner-about.min.js",
    array("jquery"),
    null,
    true
);
...
<section class="banner-about-partial-7b9ae8" <?php if($background_img === false): ?>style="<?= $banner['background_gradient']; ?>"<?php endif; ?>>
    ...
    <?php if($background_img === true): ?>
        <?= wp_get_attachment_image($banner['background_image_desktop']['ID'] ?? '', 'large', false, array(
            'class' => 'background-desktop',
            'fetchpriority' => 'high',
            'loading' => 'eager'
        )); ?>
        <?= wp_get_attachment_image($banner['background_image_movil']['ID'] ?? '', 'large', false, array(
